/**
 * This file was auto-generated by Fern from our API Definition.
 */

import * as SamsaraApi from "../index.js";

/**
 * Information about a device including its identity, last known location, last connected time, and health status.
 */
export interface DeviceResponseResponseBody {
    asset: SamsaraApi.DeviceAssetResponseResponseBody;
    health?: SamsaraApi.HealthResponseResponseBody;
    /** The last time the device was connected, in RFC 3339 format. */
    lastConnectedTime?: string;
    lastKnownLocation?: SamsaraApi.LastKnownLocationResponseResponseBody;
    /** The product model name of the device.  Valid values: `AG24`, `AG24EU`, `AG26`, `AG26EU`, `AG45`, `AG45EU`, `AG46`, `AG46EU`, `AG46P`, `AG46PEU`, `AG51`, `AG51EU`, `AG52`, `AG52EU`, `AG53`, `AG53EU`, `AT11`, `CM31`, `CM32`, `CM33`, `CM34`, `VG34`, `VG34EU`, `VG34FN`, `VG34M`, `VG54EU`, `VG54NA`, `VG55EU`, `VG55NA` */
    model: DeviceResponseResponseBody.Model;
    /** The serial number of the device. */
    serial: string;
}

export namespace DeviceResponseResponseBody {
    /**
     * The product model name of the device.  Valid values: `AG24`, `AG24EU`, `AG26`, `AG26EU`, `AG45`, `AG45EU`, `AG46`, `AG46EU`, `AG46P`, `AG46PEU`, `AG51`, `AG51EU`, `AG52`, `AG52EU`, `AG53`, `AG53EU`, `AT11`, `CM31`, `CM32`, `CM33`, `CM34`, `VG34`, `VG34EU`, `VG34FN`, `VG34M`, `VG54EU`, `VG54NA`, `VG55EU`, `VG55NA`
     */
    export type Model =
        | "AG24"
        | "AG24EU"
        | "AG26"
        | "AG26EU"
        | "AG45"
        | "AG45EU"
        | "AG46"
        | "AG46EU"
        | "AG46P"
        | "AG46PEU"
        | "AG51"
        | "AG51EU"
        | "AG52"
        | "AG52EU"
        | "AG53"
        | "AG53EU"
        | "AT11"
        | "CM31"
        | "CM32"
        | "CM33"
        | "CM34"
        | "VG34"
        | "VG34EU"
        | "VG34FN"
        | "VG34M"
        | "VG54EU"
        | "VG54NA"
        | "VG55EU"
        | "VG55NA";
    export const Model = {
        Ag24: "AG24",
        Ag24Eu: "AG24EU",
        Ag26: "AG26",
        Ag26Eu: "AG26EU",
        Ag45: "AG45",
        Ag45Eu: "AG45EU",
        Ag46: "AG46",
        Ag46Eu: "AG46EU",
        Ag46P: "AG46P",
        Ag46Peu: "AG46PEU",
        Ag51: "AG51",
        Ag51Eu: "AG51EU",
        Ag52: "AG52",
        Ag52Eu: "AG52EU",
        Ag53: "AG53",
        Ag53Eu: "AG53EU",
        At11: "AT11",
        Cm31: "CM31",
        Cm32: "CM32",
        Cm33: "CM33",
        Cm34: "CM34",
        Vg34: "VG34",
        Vg34Eu: "VG34EU",
        Vg34Fn: "VG34FN",
        Vg34M: "VG34M",
        Vg54Eu: "VG54EU",
        Vg54Na: "VG54NA",
        Vg55Eu: "VG55EU",
        Vg55Na: "VG55NA",
    } as const;
}
